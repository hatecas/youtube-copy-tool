...
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/upload-youtube`, {
      method: "POST",
    ...
    try {
      ...
      console.error("[upload-youtube] 백엔드 응답이 JSON이 아님:", text.slice(0, 500));
    ...
    console.error("[upload-youtube] 백엔드 연결 실패:", message);
    return NextResponse.json(
